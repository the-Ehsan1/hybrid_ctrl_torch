...
# Minimal PPO baseline, robust to odd action/reward shapes.
...
# Ensure project root is importable even if run as a script
...
def as_scalar(x) -> float:
    """Coerce reward-like things (tensor/array/python) to a float scalar."""
    ...
    # If env gives vector reward, use mean for baseline
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for _ in range(steps_per_epoch):
            ...
            a_flat = torch.clamp(a.squeeze(0), flat_low, flat_high).cpu().numpy()  # [act_dim]
